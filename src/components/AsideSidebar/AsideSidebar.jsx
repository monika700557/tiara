...
const AsideSidebar = () => {
  ...
  return (
    <aside className="h-screen fixed w-full">
      <nav className="flex-1 pl-10 pt-4  ml-[-20px] mt-[5rem] text-[#95979b] bg-white rounded-3xl h-full !overflow-y-scroll w-[240px]">
        <ul>
          {navLinks.map((navItem, index) => (
            <li className="mb-4" key={index}>
              <a
                href={navItem.href || "#"}
                className="hover:text-[#7638ff] hover:bg-[rgba(118,56,255,0.05)] pt-[5px] pb-[3px] flex font-semibold"
