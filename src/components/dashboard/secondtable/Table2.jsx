...
const rows = [
  { customer: 'Barbara Moore', amount: '$566', expiryDate: '23 Nov 2020', status: 'Paid' },
  { customer: 'John Doe', amount: '$150', expiryDate: '25 Nov 2020', status: 'Sent' },
  { customer: 'Jane Smith', amount: '$200', expiryDate: '27 Nov 2020', status: 'Partially Paid' },
  { customer: 'Mike Johnson', amount: '$175', expiryDate: '29 Nov 2020', status: 'Overdue' },
  { customer: 'Emily Davis', amount: '$125', expiryDate: '01 Dec 2020', status: 'Paid' },
];
...
const Table1 = () => {
  ...
  const getStatusClass = (status) => {
    switch (status.toLowerCase()) {
      case 'paid':
        return 'bg-paid-green text-[#26af48] font-bold'; 
        case 'sent':
  ...
  const segments = [
    { label: 'Sent', color: 'bg-green-500', value: 33 },
    { label: 'Draft', color: 'bg-yellow-500', value: 33 },
    { label: 'Expired', color: 'bg-red-500', value: 33.333 },
  ];
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 whitespace-nowrap">
            ...
            <tr>
                ...
                <th className="px-4 py-4 text-left text-base font-semibold">	Expiry Date</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {tableRows.map((row, index) => (
                <tr key={index} className="">
                  ...
                  <td className="px-4 py-2 font-medium text-sm">{row.expiryDate}</td>
                  <td className="px-4 py-2 font-medium text-sm">{row.amount}</td>
          
                  <td className="px-2 py-2 font-semibold    flex justify-center text-center">
                    <span className={`px-2 py-2 rounded-md  min-w-24 text-xs font-semibold ${getStatusClass(row.status)}`}>
                      {row.status}
                    </span>
                  </td>
